...
export function AppCatFab({ accessibilityLabel, disabled = false, label, onPress, style }: AppCatFabProps) {
  const colorScheme = useMeowneyColorScheme();
  const colors = colorScheme === 'light' ? lightColors : darkColors;
  const styles = useMemo(() => createStyles(colors), [colors]);
  const catColor = label ? colors.onPrimary : colors.primary;
  const plusColor = label ? colors.primary : colors.onPrimary;
    ...
    >
      {label ? (
        <View style={styles.labeledContent}>
          <View style={styles.labeledCat}>
            <Svg width={48} height={39} viewBox="0 0 1280 982">
              <G transform="translate(0,982) scale(0.1,-0.1)">
                <Path d={CAT_FACE_PATH} fill={catColor} />
              </G>
            </Svg>
            <MaterialCommunityIcons name="plus" size={22} color={plusColor} style={styles.labeledPlus} />
          </View>
          <Text style={styles.label}>{label}</Text>
        </View>
      ) : (
        <>
          <Svg width={62} height={50} viewBox="0 0 1280 982" style={styles.catShape}>
            <G transform="translate(0,982) scale(0.1,-0.1)">
              <Path d={CAT_FACE_PATH} fill={catColor} />
            </G>
          </Svg>
          <MaterialCommunityIcons name="plus" size={27} color={plusColor} style={styles.plus} />
        </>
      )}
    </Pressable>
  );
